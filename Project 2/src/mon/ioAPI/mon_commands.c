...
// used when receing sig quit signal
void exitSequence(mon_con *container, char **input_dirs, char **read_files, int amount_of_sub_dirs, int amount_of_files)
{
    // free allocated memory of the container
    destroyMonConData(*container);

    // free allocated memory of the input dirs and read files strings
    // if exists
    for(int i = 0 ; i < amount_of_sub_dirs; i++)
    {
        if (input_dirs[i]) free(input_dirs[i]);
    }

    if (input_dirs) free(input_dirs);

    for(int i = 0 ; i < amount_of_files; i++)
    {
        if (read_files[i]) free(read_files[i]);
    }

    if (read_files) free(read_files);
