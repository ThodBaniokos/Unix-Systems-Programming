...
// void functions
void printPrompt(void) { printf("travelMonitor:~$ "); }
        ...
        if(!strcmp(input[0], commands[i]))
        {
            switch (i + 1)
            {
                case travelRequest:
                    ...
                    if (trvl_request_sequence(proc_info, numMonitors, input, size, buf_size, stats) == EXIT_FAILURE) return EXIT_FAILURE;
                    deleteParsedString(input, size);
                    return EXIT_SUCCESS;
                case travelStats:
                    if (size > 5 || size < 4)
                    ...
                    if (trvl_stats_sequence(proc_info, numMonitors, all_file_descs, numMonitors * FILE_DESCRIPTORS, input, size, buf_size) == EXIT_FAILURE) return EXIT_FAILURE;
                    deleteParsedString(input, size);
                    return EXIT_SUCCESS;
                case addVacRecs:
                    if (size > 2)
                    ...
                    if (add_vac_recs_sequence(proc_info, numMonitors, input, size, buf_size) == EXIT_FAILURE) return EXIT_FAILURE;
                    deleteParsedString(input, size);
                    return EXIT_SUCCESS;
                case searchVacRecs:
                    if (size > 2)
                    ...
                    if (search_vac_status_sequence(all_file_descs, numMonitors, numMonitors * FILE_DESCRIPTORS, input, size, buf_size) == EXIT_FAILURE) return EXIT_FAILURE;
                    deleteParsedString(input, size);
                    return EXIT_SUCCESS;
                case _exit:
                    if (exit_sequence_query(proc_info, numMonitors, stats, log_path) == EXIT_FAILURE) return EXIT_FAILURE;
                    deleteParsedString(input, size);
                    return EXIT;
            }
        }
